...
@section('content')
    <div class="card">
        <div class="card-header bg-white">
            <a class="btn btn-primary ml-1 " href="/dashboard/articles/create"><span>
                    Add New Data</span></a>
        </div>
                                    ...
                                    <td>
                                        <form action="{{ route('articles.destroy', $row->id) }}" method="POST">
                                            <a class="btn btn-secondary text-white"
                                                href="{{ URL::to('dashboard/articles/' . $row->id) }}">View</a>
                                            <a class="btn btn-info text-white"
                                                href="{{ URL::to('dashboard/articles/' . $row->id . '/edit/') }}">Edit</a>
                                            @csrf
                                            @method('DELETE')
